...
import { 
  ...
  Car, Heart, Building2, Umbrella, ShoppingBag, ShieldCheck, CarFront, Landmark,
  Wallet, Banknote, LayoutGrid, FileSignature, Wrench, UserCheck, UserPlus, IdCard, Users2, Link, History,
  Cake, GitBranch, CreditCard, Stethoscope, Hourglass, FilePlus // Added FilePlus
} from 'lucide-react';

// ==========================================
// 1. GLOBAL MOCK DATA & HELPERS
// ==========================================
...
const MOCK_PARTNERS = [
  { 
    ...
    potential: 2500000, mtdAchievement: 1800000, prevYearMTD: 1500000, currYearMTD: 1800000, mtdAttempts: 45, mtdConnects: 32,
    leadsLastMonth: 15, interestedCount: 8, notInterestedCount: 2
  },
  { 
    ...
    potential: 1500000, mtdAchievement: 600000, prevYearMTD: 800000, currYearMTD: 600000, mtdAttempts: 20, mtdConnects: 8,
    leadsLastMonth: 5, interestedCount: 2, notInterestedCount: 6
  },
  { 
    ...
    potential: 800000, mtdAchievement: 100000, prevYearMTD: 200000, currYearMTD: 100000, mtdAttempts: 15, mtdConnects: 2,
    leadsLastMonth: 0, interestedCount: 0, notInterestedCount: 1
  },
  { 
    ...
    potential: 2000000, mtdAchievement: 1900000, prevYearMTD: 1200000, currYearMTD: 1900000, mtdAttempts: 50, mtdConnects: 45,
    leadsLastMonth: 22, interestedCount: 15, notInterestedCount: 3
  },
  { 
    ...
    potential: 500000, mtdAchievement: 0, prevYearMTD: 100000, currYearMTD: 0, mtdAttempts: 5, mtdConnects: 0,
    leadsLastMonth: 0, interestedCount: 0, notInterestedCount: 0
  },
  { 
    ...
    potential: 3500000, mtdAchievement: 3100000, prevYearMTD: 2800000, currYearMTD: 3100000, mtdAttempts: 60, mtdConnects: 55,
    leadsLastMonth: 35, interestedCount: 28, notInterestedCount: 1
  },
];

// --- Marketing Assets Data ---
const MOCK_MARKETING_ASSETS = [
...
const ANALYTICS_DATA = {
  ...
  segment: [
    { tier: 'Platinum', partners: 5, gwp: '₹4.2 Cr', lossRatio: '42%', retention: '94%' },
    { tier: 'Gold', partners: 12, gwp: '₹3.1 Cr', lossRatio: '58%', retention: '88%' },
    { tier: 'Silver', partners: 24, gwp: '₹1.8 Cr', lossRatio: '65%', retention: '82%' },
    { tier: 'Bronze', partners: 45, gwp: '₹0.9 Cr', lossRatio: '72%', retention: '75%' },
  ]
};
...
const CAMPAIGN_PARTNERS_DETAIL = [
...
const AI_BRIEFING = {
    ...
    talkingPoints: [
      { 
        ...
        status: 'new' // new, completed, pending
      },
      { 
        id: 2, 
        category: 'Cross-sell', 
...
const DonutChart = ({ data, size = 160, strokeWidth = 20, centerLabel = "Total", centerValue = "100%" }) => {
...
const DualAxisBarChart = ({ data, xAxisKey, barKey, lineKey, barColor = "bg-blue-500", lineColor = "border-indigo-500", height = 256, formatYAxis = (val) => val, tooltipFormat = { bar: 'Value', line: 'Count' } }) => {
  const maxBarVal = Math.max(...data.map(d => d[barKey])) || 1;
  const maxLineVal = Math.max(...data.map(d => d[lineKey])) || 1;
...
const Sidebar = ({ activeTab, setActiveTab, isCollapsed, toggleSidebar }) => {
  const menuItems = [
    ...
    { id: 'cockpit', label: 'Partner 360', icon: User, hidden: true },
...
// ... (DashboardTab, PartnersTab remain unchanged)
const DashboardTab = ({ setPartnerView }) => {
  const stats = useMemo(() => {
...
const PartnersTab = ({ setPartnerView }) => (
      ...
      <div className="flex gap-3 overflow-x-auto pb-1 flex-1">
        ...
        <div className="relative">
            <select className="appearance-none bg-slate-50 border border-slate-200 rounded-lg text-sm pl-3 pr-8 py-2 text-slate-600 font-medium focus:outline-none focus:ring-2 focus:ring-blue-500 cursor-pointer hover:bg-slate-100 transition-colors">
                ...
                <option>2</option>
                <option>3</option>
                <option>4 to 7</option>
                <option>More than 7</option>
            </select>
            <ChevronDown size={14} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none"/>
        </div>

        {/* New Filter: Call Connected */}
        <div className="relative">
            <select className="appearance-none bg-slate-50 border border-slate-200 rounded-lg text-sm pl-3 pr-8 py-2 text-slate-600 font-medium focus:outline-none focus:ring-2 focus:ring-blue-500 cursor-pointer hover:bg-slate-100 transition-colors">
    ...
    <div className="overflow-auto flex-1">
      <table className="w-full text-left border-collapse">
        <thead className="bg-slate-50 sticky top-0 z-10 shadow-sm">
          <tr>
            {['Partner', 'Tier / Status', 'Potential vs MTD Ach.', 'YoY MTD (PY vs CY)', 'Connects / Attempts', 'Leads (1M)', 'Int. / Not Int.', 'Last Disposition', 'Date', 'Action'].map((h, i) => (
        ...
        <tbody className="divide-y divide-slate-100">
          {MOCK_PARTNERS.map((p) => (
            <tr key={p.id} onClick={() => setPartnerView(p)} className="hover:bg-blue-50/40 cursor-pointer group transition-colors">
              ...
              <td className="px-6 py-4">
                  <span className="text-sm font-bold text-slate-700">{p.leadsLastMonth}</span>
              </td>
              <td className="px-6 py-4">
                  <div className="flex flex-col gap-1">
                      <span className="text-xs font-bold text-green-600 bg-green-50 px-2 py-0.5 rounded w-fit">{p.interestedCount} Int.</span>
                      <span className="text-xs font-bold text-red-600 bg-red-50 px-2 py-0.5 rounded w-fit">{p.notInterestedCount} Not Int.</span>
                  </div>
              </td>
...
// --- Partner 360 (Cockpit) ---
const Partner360 = ({ partner, onBack, onRaiseTicket }) => {
  ...
  const [activePanel, setActivePanel] = useState('metrics'); // 'metrics', 'notes', 'profile', 'menu'
  ...
  const [sellNowProduct, setSellNowProduct] = useState(null);
  ...
  const toggleTalkingPointStatus = (id, newStatus) => {
      ...
      return (
          <div className="space-y-6 animate-in fade-in">
              <div className="text-center mb-8">
                  ...
                  <p className="text-slate-500 mt-2">Select a product category to instantly generate a quote link for your partner or customer.</p>
              </div>
  ...
  const renderTabContent = () => {
      switch(activeTab) {
          case 'overview': return (
              <div className="space-y-6 animate-in fade-in">
                  {/* Detailed Performance Stats moved to Overview Module */}
                  <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
                        {/* Card 1: YoY Business MTD */}
  ...
  const renderStatsPanel = () => (
      <div className="flex flex-col h-full bg-slate-50/50">
            {/* Quick Profile Section REMOVED as per request */}
  ...
  return (
    <div className="flex flex-col h-full animate-in fade-in slide-in-from-right-8 duration-500 bg-slate-50/30">
      {/* --- 360 Header (Expanded with Details) --- */}
      <div className="bg-white border-b border-slate-200 shadow-sm flex-shrink-0 z-20">
         ...
         {/* Bottom Row: Key Details (New Request) */}
         <div className="px-6 py-2 bg-slate-50/50 border-t border-slate-100 flex flex-wrap items-center gap-x-8 gap-y-2 text-xs">
            ...
            {/* Added Customer Base to Header Details */}
            <div className="flex items-center gap-2">
      ...
      {/* --- Main Content Layout with Floating Dock --- */}
      <div className="flex flex-1 overflow-hidden">
          
          {/* Main Module Content - Flex item that shrinks when panel opens */}
          ...
          {/* Collapsible Panel - Flex item with width transition */}
          <div className={`${activePanel ? 'w-96 opacity-100 border-l' : 'w-0 opacity-0 border-l-0'} transition-all duration-300 ease-in-out bg-white border-slate-200 flex flex-col overflow-hidden shadow-xl z-30`}>
                <div className="h-full flex flex-col w-96"> {/* Fixed width inner container to prevent squashing during transition */}
                            ...
                            <div className="p-2 space-y-1">
                                {TABS.map(tab => (
                                    <button
                                        ...
                                        onClick={() => {
                                            setActiveTab(tab.id);
                                            setActivePanel(null); // Auto close menu on selection
                                        }}
...
const AnalyticsTab = () => {
  const performanceParams = [
    { label: 'Total Partners Assigned', value: '1,032', icon: Users, color: 'blue' },
    { label: 'Target vs Achievement (MTD)', value: '114%', icon: Target, color: 'green' },
    { label: 'Active Partners Today', value: '30', icon: UserCheck, color: 'indigo' },
    { label: 'Active Partner Activation', value: '30', icon: Zap, color: 'yellow' },
    { label: 'Inactive Partner Reactivation', value: '15', icon: RefreshCw, color: 'orange' },
    { label: 'MTD Business (GWP)', value: '₹6,21,468', icon: DollarSign, color: 'emerald' },
    { label: 'New Leads (Inactive)', value: '23', icon: FilePlus, color: 'cyan' },
  ];

  const efficiencyMetrics = [
    { label: 'Unique Partners Connected (MTD)', value: '372' },
    { label: 'Unique Partners Connected (FTD)', value: '25' },
    { label: 'Connectivity Rate', value: '77.1%' },
    { label: 'Avg. Talk Time (Active)', value: '232s' },
    { label: 'Avg. Response Time', value: '3.5 hrs' },
    { label: 'Call-to-Conversion', value: '5.2%' },
  ];

  const qualityMetrics = [
    { label: 'Top 20 Partner Contribution', value: '41.3%' },
    { label: 'Active Partner Engagement Rate', value: '84.7%' },
    { label: 'Inactive Partner Calls (>3 attempts)', value: '33' },
    { label: 'Active Partners Not Called', value: '50' },
    { label: 'Call Quality Score', value: '4.5/5' },
    { label: 'Partner Feedback (CSAT)', value: '4.4/5' },
  ];

  const kpiMetrics = [
    { label: 'GWP MTD', value: '₹2,28,446' },
    { label: 'Target Ach.', value: '87%' },
    { label: 'Activation Rate', value: '11%' },
    { label: 'Inactive Reactivations', value: '9' },
    { label: 'Active Retention', value: '75%' },
    { label: 'Avg. Premium', value: '₹13,327' },
    { label: 'Partner CSAT', value: '85%' },
    { label: 'Engagement Score', value: '63' },
    { label: 'Unique Partners Connected', value: '122' },
    { label: 'Call-to-Business', value: '10%' },
    { label: 'CRM Hygiene', value: '87%' },
    { label: 'System Logins (MTD)', value: '20' },
  ];
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="flex justify-between items-center bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
        <h2 className="text-xl font-bold text-slate-800 flex items-center gap-2">
          <BarChart2 className="text-blue-500" /> Performance Analytics
        </h2>
        <div className="flex gap-2">
            <span className="text-sm text-slate-500 flex items-center mr-4">Last Updated: Just now</span>
            <button className="px-4 py-1.5 border border-slate-200 rounded-lg text-sm font-medium hover:bg-slate-50 text-slate-600 flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {performanceParams.map((item, index) => (
              <div key={index} className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex items-center justify-between hover:shadow-md transition-shadow">
                  <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          {/* Efficiency & Calling Module */}
          <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
              <h3 className="font-bold text-slate-800 mb-4 flex items-center gap-2">
                  <PhoneCall size={18} className="text-blue-500"/> Calling Efficiency
              </h3>
          ...
          <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
              <h3 className="font-bold text-slate-800 mb-4 flex items-center gap-2">
                  <ShieldCheck size={18} className="text-green-500"/> Quality & Engagement
              </h3>
              <div className="space-y-4">
          ...
          <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm flex flex-col items-center justify-center">
              ...
              <div className="py-2">
                  <DonutChart data={ANALYTICS_DATA.mix} size={160} strokeWidth={20} centerValue="100%" centerLabel="Total Mix" />
              </div>
              <div className="w-full grid grid-cols-2 gap-3 mt-4">
                {ANALYTICS_DATA.mix.map((item, i) => (
                    <div key={i} className="flex items-center gap-2"><div className={`w-2 h-2 rounded-full ${item.tailwindClass}`}></div><div className="flex-1"><p className="text-[10px] text-slate-500">{item.name}</p><p className="text-xs font-bold text-slate-800">{item.value}%</p></div></div>
                ))}
              </div>
          </div>
      </div>
      ...
      <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
          <h3 className="font-bold text-slate-800 mb-6 flex items-center gap-2">
              <Activity size={18} className="text-purple-500"/> Key Performance Indicators
          </h3>
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-y-8 gap-x-6">
              {kpiMetrics.map((kpi, i) => (
                  <div key={i} className="text-center relative">
                      {i % 6 !== 5 && <div className="absolute right-0 top-1/2 -translate-y-1/2 h-8 w-px bg-slate-100 hidden lg:block"></div>}
                      <p className="text-[10px] text-slate-500 uppercase font-bold tracking-wide h-8 flex items-center justify-center mb-1">{kpi.label}</p>
      ...
      <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
        ...
        <DualAxisBarChart data={ANALYTICS_DATA.trend} xAxisKey="month" barKey="premium" lineKey="policies" formatYAxis={(val) => `₹${val.toFixed(1)} Cr`} tooltipFormat={{ bar: 'GWP (Cr)', line: 'Policies' }} />
      </div>
    </div>
  );
};
...
const CommunicationHub = () => {
  ...
  const [selectedComm, setSelectedComm] = useState(MOCK_COMMS[0]);
...
const CreateTicketForm = ({ preFillData, onClose, onSubmit }) => {
...
const TicketsTab = ({ preFillPartner, clearPreFill }) => {
    ...
    return (
        <div className="h-full flex flex-col gap-4 animate-in fade-in duration-500">
            ...
            <div className="flex-1 overflow-hidden">
                {view === 'list' && <TicketList onSelectTicket={(t) => { setSelectedTicket(t); setView('detail'); }} />}
...
export default function PartnerApp() {
  ...
  const [selectedPartner, setSelectedPartner] = useState(MOCK_PARTNERS[0]); // Default to first partner
  ...
  const handleRaiseTicketFrom360 = (partner) => {
